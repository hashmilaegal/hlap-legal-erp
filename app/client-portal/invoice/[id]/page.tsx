...
export default function ClientInvoicePage() {
  ...
  async function fetchInvoice() {
    const { data } = await supabase.from('invoices').select('*, clients(name, email, phone), matters(title)').eq('id', invoiceId).single()
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="px-8 py-6 border-b">
            <div className="flex justify-between"><div><p className="text-sm text-gray-600">Invoice #</p><p className="text-xl font-bold">{invoice.invoice_number}</p><p className="text-sm text-gray-600 mt-2">Date: {new Date(invoice.invoice_date).toLocaleDateString()}</p><p className="text-sm text-gray-600">Due: {new Date(invoice.due_date).toLocaleDateString()}</p></div><div><span className={`px-3 py-1 rounded-full text-sm font-medium inline-block ${invoice.status === 'paid' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}`}>{invoice.status.toUpperCase()}</span></div></div>
          </div>
          <div className="px-8 py-6 border-b bg-gray-50"><p className="text-sm font-semibold">BILL TO:</p><p className="font-bold">{invoice.clients?.name}</p><p>Email: {invoice.clients?.email}</p><p>Phone: {invoice.clients?.phone}</p></div>
